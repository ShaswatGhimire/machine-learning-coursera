...
J = sum(sum(((-1) * y2 .* log(H)) - ((1-y2) .* log(1-H))))* (1/m);
...
for i = 1 : m
  a1 = X(i, 1:end);  %a1 = 1*401
  z2 =  a1 * Theta1';  %z2 = 1 * 25
  z2_big = [1, sigmoidGradient(z2)];
  a2 = sigmoid(z2); %a2 = 1 * 26
  a2 = [1 , a2];
  z3 = a2 * Theta2'; %a3 = 1 * 10
  a3 = sigmoid(z3);
  
  ym = (1 : num_labels) == y(i);  %1*10
  %size(a3)
  %size(ym)
  d3 = a3 - ym;
  %size(d3);
  d2 = (d3 * Theta2)  .*  z2_big ; %d2 = 1* 26
  %size(d2);
  
  d2_skip = d2(1:end, 2:end);  %d2_skip = 10 * 25
  Theta1_grad = Theta1_grad + d2_skip' * a1; %25*1 * 1*401
